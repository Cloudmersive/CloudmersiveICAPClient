...
{
    public class CloudmersiveIcapClient : IDisposable
    {
        ...
        private const String USERAGENT = "Cloudmersive ICAP Client/1.1";
        private const String ICAPTERMINATOR = "\r\n\r\n";
        private const String HTTPTERMINATOR = "0\r\n\r\n";

        public string Method = "RESPMOD";
        ...
        public CloudmersiveIcapClient(String serverIP, int port, String icapService, bool sendOptions, int previewSize = -1)
        {
        ...
        public bool scanFileWithPreview(String filepath)
        {
            using (FileStream fileStream = new FileStream(filepath, FileMode.Open))
            {
                ...
                string transmit = "RESPMOD icap://" + serverIP + "/" + icapService + " ICAP/" + VERSION + "\r\n"
                    + "Host: " + serverIP + "\r\n"
                ...
                if (fileSize > previewSize)
                {
                    ...
                    while ((n = fileStream.Read(buffer, 0, stdSendLength)) > 0)
                    {
                        ...
                        actualText += Encoding.ASCII.GetString(buffer);
                    }
        ...
        public bool scanFileWithPreview(byte[] fileData)
        {
        ...
        public int scanFileWithoutPreview(byte[] fileData)
        {
            using (MemoryStream fileStream = new MemoryStream(fileData))
            {
                int bytesSent = 0;
                ...
                string transmit = ToUpper(Method) + " icap://" + serverIP + "/" + icapService + " ICAP/" + VERSION + "\r\n"
                    + "Host: " + serverIP + "\r\n"
                    + "User-Agent: " + USERAGENT + "\r\n"
                    + "Allow: 204\r\n"
                    + "Encapsulated: res-hdr=0, res-body=" + resBody.Length + "\r\n"
                ...
                bytesSent += requestBuffer.Length;
                ...
                if (fileSize > previewSize)
                {
                    ...
                    while ((n = fileStream.Read(buffer, 0, stdSendLength)) > 0)
                    {
                        ...
                        byte[] bytesToSend = Encoding.ASCII.GetBytes(buffer.Length.ToString("X") + "\r\n");
                        sender.Send(bytesToSend);
                        bytesSent += bytesToSend.Length;

                        sender.Send(buffer.Take(n).ToArray());
                        bytesSent += n;

                        bytesToSend = Encoding.ASCII.GetBytes("\r\n");
                        sender.Send(bytesToSend);
                        bytesSent += bytesToSend.Length;
                ...
                if (tempString != null)
                {
                    ...
                    if (status == 200)
                    {
                        string body = getHeader(HTTPTERMINATOR);
        ...
        public int scanFileWithPreview(byte[] fileData, out string httpResponseBody)
        {
            httpResponseBody = "";
            ...
            using (MemoryStream fileStream = new MemoryStream(fileData))
            {
                ...
                string transmit = ToUpper(this.Method) + " icap://" + serverIP + "/" + icapService + " ICAP/" + VERSION + "\r\n"
                    + "Host: " + serverIP + "\r\n"
                    + "User-Agent: " + USERAGENT + "\r\n"
                    + "Allow: 204\r\n"
                    + "Preview: " + previewSize + "\r\n"
        ...
        public static string ToUpper(string destination)
        {
        ...
        public static void Write(Stream outputStream, byte[] bytes)
        {
            outputStream.Write(bytes, 0, bytes.Length);
        }
        public byte[] GenerateRequestWithPreview(byte[] fileData)
        {
            ...
            using (MemoryStream fileStream = new MemoryStream(fileData))
            {
                ...
                output.Write(requestBuffer, 0, requestBuffer.Length);
                ...
                if (fileSize <= previewSize)
                {
                    Write(output, Encoding.ASCII.GetBytes("0; ieof\r\n\r\n"));
                }
                else if (previewSize != 0)
                {
                    Write(output, Encoding.ASCII.GetBytes("0\r\n\r\n"));
                }
                ...
                if (fileSize > previewSize)
                {
                    ...
                    {
                        offset += n;  // offset for next reading
                        Write(output, Encoding.ASCII.GetBytes(buffer.Length.ToString("X") + "\r\n"));
                        Write(output, buffer);
                        Write(output, Encoding.ASCII.GetBytes("\r\n"));
        ...
        public byte[] GenerateRequestWithoutPreview(byte[] fileData)
        {
            using (MemoryStream output = new MemoryStream())
            using (MemoryStream fileStream = new MemoryStream(fileData))
            {
                ...
                Helpers.Write(output, chunk);
                Helpers.Write(output, Encoding.ASCII.GetBytes("\r\n"));

                if (fileSize <= previewSize)
                {
                    Helpers.Write(output, Encoding.ASCII.GetBytes("0; ieof\r\n\r\n"));
                }
                else if (previewSize != 0)
                {
                    Helpers.Write(output, Encoding.ASCII.GetBytes("0\r\n\r\n"));
                }*/
